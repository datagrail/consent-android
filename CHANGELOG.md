...
- `setCcpaOptout(optedOut[, callback])` and `getCcpaOptout()`: a first-class CCPA/CPRA "Do Not Sell or Share" choice. Android has no native DNSMPI signal, so the host app is the source of truth. The flag is stored on the device and never changes a category. It is written through to the user's universal consent record when universal consent is enabled, `universalConsent.sync_optout` is on, `setUserIdentifier` has succeeded in this process, and an explicit consent choice is stored. Otherwise it rides the next universal consent write (TRUST-2591)

### Changed

- **Behavior change:** universal consent writes now send `ccpa_optout` from the user's stored choice (`setCcpaOptout`), gated by `universalConsent.sync_optout`. Before, Android always sent `false`. The value is never derived from the ad-tracking signal or a category. When a login finds a record, the record's `ccpa_optout` replaces the local flag. On a re-sync it is adopted only when `sync_optout` is on. `clearUserIdentifier()` resets the flag to `false` and `reset()` wipes it. Records written earlier are adopted as they are (TRUST-2591)
